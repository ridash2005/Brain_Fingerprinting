...
OUTPUT_DIR = ensure_dir(os.path.join("results", "runs", f"{RUN_ID}_refine_network_avg_{args.task}_{args.network}"))
...
fc_task= np.load(fc_task_path)  # Load the FC data for the specified task
...
# Save the figure
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, f"{args.task}_{args.network}_avg_fcs.png"))
plt.close()
